...
function buildDetailsReport({ geoData, userAgent, fp, webrtcIps, ip, screenSize, width, height, platform, uaData }) {
  let detailsMsg = '';
  ...
  if (uaData) {
    detailsMsg += `Браузер: ${uaData.browser || 'неизвестно'}, ОС: ${uaData.os || 'неизвестно'}\n`;
    detailsMsg += `Бренд: ${uaData.brand || 'неизвестно'}\n`;
    detailsMsg += `Тип устройства: ${uaData.device || 'неизвестно'}\n`;
  }
  if (geoData && geoData.lat && geoData.lon) {
    detailsMsg += `Координаты: ${geoData.lat}, ${geoData.lon}\n`;
  }
